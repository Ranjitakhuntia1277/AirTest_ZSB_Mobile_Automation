...
'DieID', 'EthernetAddress', 'FirmwareVersion', 'HardwareModel', 'HardwarePlatform', 'HasSiDP', 'HostAttached', 'MLBSerialNumber', 'ModelNumber', 'NonVolatileRAM', 'PartitionType', 'PasswordProtected', 'ProductName', 'ProductType', 'ProductVersion', 'ProductionSOC', 'ProtocolVersion', 'RegionInfo', 'SerialNumber', 'SoftwareBehavior', 'SoftwareBundleVersion', 'SupportedDeviceFamilies', 'TelephonyCapability', 'TimeIntervalSince1970', 'TimeZone', 'TimeZoneOffsetFromUTC', 'TrustedHostAttached', 'UniqueChipID', 'UniqueDeviceID', 'UntrustedHostBUID', 'UseRaptorCerts', 'Uses24HourClock', 'WiFiAddress', 'WirelessBoardSerialNumber']
    ...
    def Show_popup_To_SignIn_PhoneA_And_B_Without_Printer_Manually(self):
        ...
        root.withdraw()  # Hide the root window
        root.attributes('-topmost', True)
        messagebox.showinfo("Notification", "Prepare two production users without adding any printer and sign in phone A and B Manually")
        root.destroy()
    ...
    def Show_popup_To_Connect_To_PhoneB_Manually(self):
        ...
        root.withdraw()  # Hide the root window
        root.attributes('-topmost', True)
        messagebox.showinfo("Notification", "Use phone B, click the hamburger Icon and click on 'Add a Printer', click Start button Manually")
        root.destroy()
    ...
    def Show_popup_To_Verify_Target_Printer_Is_Not_Displaying_On_Select_Your_PrinterPage_Manually(self):
        ...
        root.withdraw()  # Hide the root window
        root.attributes('-topmost', True)
        messagebox.showinfo("Notification", "Verify Target Printer Is Not Displaying On Select Your PrinterPage Manually")
        root.destroy()
    ...
    def Show_popup_To_Verify_No_Duplicat_Essids_And_Other_Details_Manually(self):
        ...
        root.withdraw()  # Hide the root window
        root.attributes('-topmost', True)
        messagebox.showinfo("Notification", "Check no duplicated Essids, all Essid is unique and only wpa psk and open securiy Essids in it Manually")
        root.destroy()
    ...
    def Show_popup_To_Make_The_Printer_To_Pairing_Mode_Manually(self):
        ...
        root.withdraw()  # Hide the root window
        root.attributes('-topmost', True)
        messagebox.showinfo("Notification", "Press target printer 3-5 second to enter pairing mode(blue light) Manually")
        root.destroy()
    ...
    def Show_popup_To_Verify_Connecting_To_Wifi_Network_Page_Fonts_Manually(self):
        ...
        root.withdraw()  # Hide the root window
        root.attributes('-topmost', True)
        messagebox.showinfo("Notification", "Connecting to Wi-Fi Network page displayed with new fonts and style Manually")
        root.destroy()
    ...
    def Show_popup_To_Select_A_WIFI_Without_Network_And_Chcek_The_Fonts_And_Style_Manually(self):
        ...
        root.withdraw()  # Hide the root window
        root.attributes('-topmost', True)
        messagebox.showinfo("Notification", "Select a Wi-Fi without network and verify the new fonts and style Manually")
        root.destroy()
    ...
    def Show_popup_To_Verify_Bluetoothpairing_your_printer_page_with_new_fonts_and_Style(self):
        ...
        root.withdraw()  # Hide the root window
        root.attributes('-topmost', True)
        messagebox.showinfo("Notification", "Verify Bluetoothpairing your printer page with new fonts and Style Manually")
        root.destroy()
    def Show_popup_To_Verify_Unable_To_Connect_To_Printer_page_with_new_fonts_and_Style(self):
        root = tk.Tk()
        root.withdraw()  # Hide the root window
        root.attributes('-topmost', True)
        messagebox.showinfo("Notification","Verify Unable To Connect To Printer page with new fonts and Style Manually")
        root.destroy()
    ...
    def Show_popup_To_Verify_Bluetooth_Permission_Popup_With_New_Design_On_The_Device_Manually(self):
        ...
        root.withdraw()  # Hide the root window
        root.attributes('-topmost', True)
        messagebox.showinfo("Notification", "Check the Bluetooth permission dialog display correctly with the new fonts and styles Manually")
        root.destroy()
    ...
    def Show_popup_To_Verify_the_view_of_pdf_screens_for_1stone_and_for_rest_it_will_be_different_Manually(self):
        ...
        root.withdraw()  # Hide the root window
        root.attributes('-topmost', True)
        messagebox.showinfo("Notification", "Verify Manually the view of pdf screens for 1st one it will be different and for rest it will be different")
        root.destroy()


#     ###"""""""""to show the pop up and input the text what to srearch and perform
